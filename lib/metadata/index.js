...
function getGoals(account, property, view) {
  var urls = [];
  var goal_names = [];
  var x = requestGoals(account.id, property.id, view.id).then(function(data) {
    for(var i=0; i < data.result.items.length; i++) {
      if ((data.result.items[i].type === 'URL_DESTINATION') && (data.result.items[i].urlDestinationDetails)) {
        urls.push(data.result.items[i].urlDestinationDetails.url);
        goal_names.push(data.result.items[i].name);
      }
    }
    window.goal_urls = urls;
    window.goal_names = goal_names ;
  });
  return urls;
}
...
module.exports = {
  ...
  getGoals: function(account, property, view) {
